...
import { StudioQuality, StudioOption, PendingNaming } from '@/engine/types';
import { STUDIO_OPTIONS } from '@/hooks/useGame';

interface StudioSelectionModalProps {
...
export function StudioSelectionModal({
  pending,
  playerMoney,
  onSelectStudio,
  onCancel,
}: StudioSelectionModalProps) {
  // Only show for studio-selection type
  if (pending.type !== 'studio-selection') {
    return null;
  }

  const recordType = pending.recordAction === 'RECORD_SINGLE' ? 'single'
    : pending.recordAction === 'RECORD_EP' ? 'EP' : 'album';
  const weeks = pending.recordAction === 'RECORD_SINGLE' ? 1
    : pending.recordAction === 'RECORD_EP' ? 2 : 4;
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-lg max-w-lg w-full p-6 shadow-2xl">
        ...
        <div className="mb-6 text-center">
          ...
          <p className="text-sm text-gray-400 mt-2">
            Recording a {recordType} ({weeks} week{weeks > 1 ? 's' : ''}) • {pending.songIds.length} track{pending.songIds.length > 1 ? 's' : ''}
          </p>
        </div>
        ...
        <div className="space-y-3 mb-6">
          {STUDIO_OPTIONS.map((option) => {
            const totalCost = option.costPerWeek * weeks;
